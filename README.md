...
the version prefix but stay under the base. `client.wsUrl` is the same base with
the scheme swapped, prefix included, so the stream can never end up on a
different host than the REST calls.
